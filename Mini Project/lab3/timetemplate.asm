...
############################Surprise assignment
	...
	bnez	$t0, notHour	# branchar då det inte är 00:00
	nop
  ...
  hour:	.ascii "HOUR"
	...
	lw	$t1, ($t0)	# loadar $t1 med värdet på addressen till "HOUR" (värdet: "HOUR"
	sw	$t1, ($s0)	# lagrar sedan värdet i addressen som skrivs ut, då "HOUR" i ASCII består bara av 4 bytes.
	sb	$zero, 4($s0)	# lägger till null efter alla bytes som formar "HOUR" för att inte skriva ut mer saker.
  	j	endTime
  	nop
############################
 ...
 notHour:		
	...
	sra	$a0, $s1, 0	# Skriver om v som a för att sedan få LSB
	jal	hexasc
	nop
